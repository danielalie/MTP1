#include<stdio.h>//TERMINARRRREREIU CDJVIODJVINDCVJDNC AVEEEE
#include<math.h>//para usar o floor

struct Pixel
{
  unsigned char B;// usando so para byte R - azul
...
{
    struct Pixel pixel;
    int WPX, HPX, BPP, NCP, NIC, PAL, lap; // siglas para largura, altura, etc
    ...
    fread (&byte, sizeof(byte),1,imagem); printf("%c", byte);//2 bytes	0-1	Se for Windows, 'B' e 'M'
    fread (&dword, sizeof(dword),1,imagem);printf("tamanho do arquivo (bytes): %u\n", dword);//dword 2-5 Tamanho do arquivo (bytes)
    fread (&word, sizeof(word),1,imagem); printf("informacao reservada: %08X\n", word);//word 6-9 Informação reservada. se quisesse pular esses bytes: fseek(imagem,4 (pula 4 bytes),SEEK_CUR);
    ...
    printf("\n******PIXELS******\n");
    lap = 0;
    PAL = floor ((BPP*WPX+31.)/32.)*4; printf("tamanho da linha da array de pixels (bytes):%u\n", PAL);//Leitura dos pixels
    while (!feof(imagem))
    {
